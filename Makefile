...
create-dbuser:
	...
	docker compose exec mongos1 mongosh --eval 'db.getSiblingDB("$(UGC_MONGO_DB)").createUser({ user: "$(UGC_MONGO_USER)", pwd: "$(UGC_MONGO_PASSWORD)", roles: [{ role: "readWrite", db: "$(UGC_MONGO_DB)" }] })'
